  ...
  return (
    <div className="min-h-screen bg-[#1E1F23] text-gray-100 p-6">
      ...
      <section className="mb-10">
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-6">
          ...
          <div className="flex flex-col w-40">
            ...
            <select value={filters.camera} onChange={(e) => handleChange("camera", e.target.value)} className="px-3 py-3 border border-gray-700 rounded-lg bg-[#2A2B30] shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-[#5388DF] text-gray-200">
              <option value="">All Camera</option>
              {cameraOptions.map((cam) => (
                <option key={cam.id || cam.name || cam} value={cam.name || cam}>
                  {cam.name || cam}
                </option>
              ))}
            </select>
          ...
          <div className="flex flex-col w-40">
            ...
            <select value={filters.sortBy} onChange={(e) => handleChange("sortBy", e.target.value)} className="px-3 py-3 border border-gray-700 rounded-lg bg-[#2A2B30] shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-[#5388DF] text-gray-200">
          ...
          <div className="flex flex-col w-40">
            <label className="font-medium text-sm mb-1 text-gray-400">Status</label>
            <select value={filters.status} onChange={(e) => handleChange("status", e.target.value)} className="px-3 py-3 border border-gray-700 rounded-lg bg-[#2A2B30] shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-[#5388DF] text-gray-200">
      ...
      <section>
        ...
        <p className="text-gray-600 text-sm mb-6">All recorded incidents are listed below.</p>
        ...
        <div className="space-y-3">
          {currentItems.map((n) => (
            <div key={n.id} className="grid grid-cols-7 gap-4 bg-[#2A2B30] rounded-lg shadow-sm border border-gray-700 p-4 hover:bg-[#3A3B40] transition-colors items-center">
              ...
              <div className="text-center">
                <button onClick={() => handleView(n)} className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-[#5388DF] rounded-md hover:bg-[#19325C] transition-colors">
        ...
        <div className="flex justify-center mt-6 space-x-2">
          <button onClick={() => setCurrentPage((prev) => Math.max(1, prev - 1))} disabled={currentPage === 1} className="px-4 py-2 bg-[#2A2B30] text-gray-200 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#19325C] transition">
            Previous
          </button>
          {Array.from({ length: totalPages }, (_, i) => (
            <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`px-4 py-2 rounded-lg ${currentPage === i + 1 ? "bg-[#5388DF] text-white" : "bg-[#2A2B30] text-gray-200 hover:bg-[#19325C]"} transition`}>
              {i + 1}
            </button>
          ))}
          <button onClick={() => setCurrentPage((prev) => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className="px-4 py-2 bg-[#2A2B30] text-gray-200 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#19325C] transition">
            Next
          </button>
        </div>
      </section>
