...
export default function Incident() {
  const notifications = [
    { id: 1, camera: "Camera A", violation: "No Helmet", status: "Pending",  date: "2025-09-27 10:35 AM", worker: "Worker 1" },
    { id: 2, camera: "Camera B", violation: "No Vest",   status: "Resolved", date: "2025-09-26 4:20 PM",  worker: "Worker 2" },
  ];
  ...
  const [filters, setFilters] = useState({ camera: "", violation: "", status: "" });
    ...
    <div className="min-h-screen bg-gray-50 text-[#19325C] p-6">
      ...
      <section>
        ...
        <div className="grid grid-cols-6 gap-4 mb-4 text-xs md:text-sm font-semibold text-white">
          <div className="bg-[#19325C] px-4 py-2 rounded-lg">Worker</div>
        ...
        <div className="space-y-3">
          {notifications.map((n) => (
            <div
              key={n.id}
              className="grid grid-cols-6 gap-4 bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow"
            >
              <div className="font-medium">{n.worker}</div>
              <div>{n.camera}</div>
              <div>{n.violation}</div>
