...
def init_pose():
    return mp_pose.Pose(static_image_mode=False, model_complexity=1, smooth_landmarks=True, enable_segmentation=False, min_detection_confidence=0.5, min_tracking_confidence=0.5)
...
def check_ppe(boxes_by_class, person_bbox, landmarks, xoff, yoff, cw, ch):
    ...
    lwx,lwy,lwv = abs_lm(landmarks, LM.LEFT_WRIST.value, xoff, yoff, cw, ch)
    rwx,rwy,rwv = abs_lm(landmarks, LM.RIGHT_WRIST.value, xoff, yoff, cw, ch)
    lax,lay,lav = abs_lm(landmarks, LM.LEFT_ANKLE.value, xoff, yoff, cw, ch)
    rax,ray,rav = abs_lm(landmarks, LM.RIGHT_ANKLE.value, xoff, yoff, cw, ch)
    try:
